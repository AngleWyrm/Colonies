...
public class TileEntityColonyChest extends TileEntity implements IInventory {
  ...
  public TileEntityColonyChest() {
    this(ColonyChestType.TOWNHALL);
  }

  protected TileEntityColonyChest(ColonyChestType type) {
    super();
    this.type = type;
  ...
  public ColonyChestType getType() {
    return type;
  }
  ...
  public void updateEntity() {
    ...
    if ((++ticksSinceSync % 20) * 4 == 0) {
      worldObj.addBlockEvent(xCoord, yCoord, zCoord, ColonyChest.ColonyChestBlock.blockID, 3, ((numUsingPlayers << 3) & 0xF8) | (facing & 0x7));
      if (inventoryTouched) {
  ...
  public void openChest() {
    if (worldObj == null)
      return;
    numUsingPlayers++;
    worldObj.addBlockEvent(xCoord, yCoord, zCoord, ColonyChest.ColonyChestBlock.blockID, 1, numUsingPlayers);
  }
  ...
  public TileEntityColonyChest updateFromMetadata(int l) {
    if (worldObj != null && worldObj.isRemote) {
      if (l != type.ordinal()) {
        worldObj.setBlockTileEntity(xCoord, yCoord, zCoord, ColonyChestType.makeEntity(l));
        return (TileEntityColonyChest) worldObj.getBlockTileEntity(xCoord, yCoord, zCoord);
      }
    }
    return this;
  }
  ...
  public void handlePacketData(int typeData, int[] intData) {
    TileEntityColonyChest chest = this;
    if (this.type.ordinal() != typeData) {
      chest = updateFromMetadata(typeData);
    }
    if (ColonyChestType.values()[typeData].isTransparent() && intData != null) {
      int pos = 0;
