...
public class BlockColonyChest extends BlockContainer {

  private Random random;

  public BlockColonyChest(int id) {
    super(id, Material.iron);
    setBlockName("TownhallChest");
    setHardness(3.0F);
  ...
  public String getTextureFile() {
    return "/pmardle/gfx/block_textures.png";
  }
  ...
  public int getBlockTextureFromSideAndMetadata(int i, int j) {
    ColonyChestType typ = ColonyChestType.values()[j];
    switch (i) {
    case 0:
    case 1:
  ...
  public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer player, int i1, float f1, float f2, float f3) {
    ...
    player.openGui(ColonyChest.instance, ((TileEntityColonyChest) te).getType().ordinal(), world, i, j, k);
    return true;
  }
  ...
  public void onBlockPlacedBy(World world, int i, int j, int k, EntityLiving entityliving) {
    ...
    TileEntity te = world.getBlockTileEntity(i, j, k);
    if (te != null && te instanceof TileEntityColonyChest) {
      ((TileEntityColonyChest) te).setFacing(chestFacing);
      world.markBlockNeedsUpdate(i, j, k);
    }
  }
  ...
  {
    TileEntityColonyChest tileentitychest = (TileEntityColonyChest) world.getBlockTileEntity(i, j, k);
  ...
  @SuppressWarnings({ "rawtypes", "unchecked" })
	...
	public void getSubBlocks(int par1, CreativeTabs par2CreativeTabs, List par3List) {
	    for (ColonyChestType type : ColonyChestType.values()) {
