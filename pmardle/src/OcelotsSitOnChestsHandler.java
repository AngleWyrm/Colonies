	...
	public void changeSittingTaskForOcelots(LivingEvent.LivingUpdateEvent evt) {
		if (evt.entityLiving.ticksExisted < 5 && evt.entityLiving instanceof EntityOcelot)
		{
			...
			for (int i=0; i<tasks.size(); i++)
			{
				...
				if (task.priority == 6 && (task.action instanceof EntityAIOcelotSit) && !(task.action instanceof ColonyChestAIOcelotSit))
				{
					task.action = new ColonyChestAIOcelotSit(ocelot, 0.4F);
				}
			}
		}
	}
